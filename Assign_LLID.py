...
temp_location = "C:/"
workspace = "C:/assign_llid_temp.gdb"
temp_gdb = "assign_llid_temp.gdb"
final_gdb = "Master_List_of_Stations_Results_Tol12000_II.gdb"
original_sampling_stations = "E:/GitHub/ToxicsRedo/StationsToLocate/FinalList/ShapeFiles/Master_List_of_Stations.shp"
...
#Add QC field to table
arcpy.AddField_management(output_table, "QAQC", "STRING")

#Now, use character matching to verify that successful rows have matching stream names.
...
with arcpy.da.UpdateCursor(output_table, [station_river_name_field,'RID','QAQC']) as cursor:
    for row in cursor:
...
arcpy.SelectLayerByAttribute_management(qc_lyr, "NEW_SELECTION", """ "QAQC" = 'Yes' """)
...
if int(arcpy.GetCount_management(qc_lyr).getOutput(0)) == len(successful_features):
    ...
    arcpy.CopyFeatures_management(qc_lyr, (temp_location + final_gdb + "/" + qc_success))
    arcpy.SelectLayerByAttribute_management(qc_lyr, "NEW_SELECTION", """ "QAQC" = 'Needs Review' """)
...
#arcpy.Delete_management((temp_location + temp_gdb))